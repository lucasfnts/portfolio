<template>
  <div class="relative overflow-hidden bg-white pt-12 sm:pt-32 lg:pt-20">
    <div
      class="mx-auto max-w-md px-6 text-center sm:max-w-3xl lg:max-w-7xl lg:px-8"
    >
      <div>
        <h2 class="text-lg font-semibold text-indigo-600">Front-End</h2>
        <p
          class="mt-2 text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl"
        >
          Site E-Commerce
        </p>
        <p class="mx-auto mt-5 max-w-prose text-xl text-gray-600">
          J'ai mis en place un site E-Commerce pour un Salon de Tatouage &
          Piercing.<br />
          La prestation comprends:
        </p>
        <ul class="list-disc py-3 text-start lg:ml-80 sm:px-px">
          <li>L'achat du nom de domaine</li>
          <li>L'hébergement et sa configuration</li>
          <li>
            La configuration des adresses mails nécéssaires à l'exploitation
            d'une boutique en ligne
          </li>
          <li>La configuration de la boutique en ligne</li>
          <li>La gestion des cookies conforme au R.G.P.D</li>
          <li>
            Le suivi statistique via une solution
            <strong>Open-Source & respectueuse de la vie privée</strong>
          </li>
        </ul>
      </div>
      <div class="mt-24">
        <div class="lg:mx-auto lg:max-w-7xl lg:gap-24 lg:px-4">
          <div
            class="mx-auto max-w-xl px-6 lg:col-start-2 lg:mx-0 lg:max-w-none lg:px-0"
          >
            <div>
              <div class="mt-6">
                <h3 class="text-xl font-bold tracking-tight text-gray-900">
                  <em>Stack</em>
                </h3>
                <p class="mt-4 text-lg text-gray-500">
                  <em>Hébergement, nom de domaine & mail</em>: Infomaniak
                  <br />
                  <em> Site E-Commerce</em>: Wordpress + WooCommerce
                  <br />
                  <em> Passerelle de payement</em>: Mollie
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="mt-12 -mb-10 sm:-mb-24 lg:mb-20">
        <img
          class="rounded-lg shadow-xl ring-1 ring-black ring-opacity-5"
          src="../assets/ecommerce.png"
          alt=""
        />
      </div>
    </div>
  </div>
</template>
